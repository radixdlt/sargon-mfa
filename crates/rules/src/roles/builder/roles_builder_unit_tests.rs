...
fn test_duplicates_not_allowed<const R: u8>(
    sut: RoleBuilder<R>,
    list: FactorListKind,
...
fn confirmation_validate_basic_violation() {
    let sut =
        ConfirmationRoleBuilder::with_factors(1, vec![], vec![FactorSourceID::sample_ledger()]);
    let res = sut.validate();
    assert_eq!(
        res,
...
fn recovery_validate_basic_violation() {
    let sut = RecoveryRoleBuilder::with_factors(1, vec![], vec![FactorSourceID::sample_ledger()]);
    let res = sut.validate();
    assert_eq!(
        res,
...
fn primary_validate_not_yet_valid_for_threshold_greater_than_threshold_factors() {
    let sut = PrimaryRoleBuilder::with_factors(1, vec![], vec![FactorSourceID::sample_ledger()]);
    let res = sut.validate();
    assert_eq!(
        res,
