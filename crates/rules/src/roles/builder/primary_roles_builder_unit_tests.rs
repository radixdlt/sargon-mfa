...
mod threshold_suite {
    ...
    fn remove_lowers_threshold_from_3_to_1() {
        ...
        sut.add_factor_source_to_threshold(FactorSourceID::sample_arculus_other())
            .unwrap();
    ...
    mod threshold_in_isolation {
        ...
        fn duplicates_not_allowed() {
            ...
            sut.add_factor_source_to_threshold(FactorSourceID::sample_device())
                .unwrap();
            _ = sut.set_threshold(2);
            test_duplicates_not_allowed(sut, FactorListKind::Threshold, sample());
        }
...
mod ledger {
    ...
    mod override_in_isolation {
        ...
        fn two_different_is_ok() {
            ...
            // Assert
            let expected =
                RoleWithFactorSourceIds::primary_with_factors(0, [], [sample(), sample_other()]);
            assert_eq!(sut.build().unwrap(), expected);
        }
    }
}
...
mod arculus {
    ...
    mod threshold_in_isolation {
        ...
        fn two_different_is_ok() {
            ...
            // Assert
            let expected =
                RoleWithFactorSourceIds::primary_with_factors(1, [sample(), sample_other()], []);
            assert_eq!(sut.build().unwrap(), expected);
        }
    }
