...
impl SplitFactorInstancesFromCache {
    ...
    pub(super) fn derive_more_requests(
        self,
    ) -> Option<IndexSet<DeriveMoreToSatisfyOriginalRequest>> {
...
pub(super) fn split_cache_response(
    ...
    for outcome in take_from_cache_outcome.outcomes().into_iter() {
        match outcome.action() {
            ...
            Action::CacheIsEmpty { requested_quantity } => {
                derive_more_requests.insert(
                    DeriveMoreToSatisfyOriginalRequest::WithoutKnownLastIndex {
                        without_known_last_index: outcome.request,
                        requested_quantity,
                    },
                );
            }
